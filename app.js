...
const selectballContainer = document.querySelector(
	'.choice_container .wrapper'
);
...
function clickNumber(e) {
	if (e.target.classList.contains('ball')) {
		...
		if (
			...
			!otherNumbers.includes(selectedNumber)
		) {
...
function clickSelectNumber(e) {
	if (e.target.classList.contains('ball')) {
		...
		if (
			otherNumbers.indexOf(otherNumber) === -1 &&
			!selectedNumbers.includes(otherNumber)
		) {
